...
public class prueba_de_ciclismo_activity extends AppCompatActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		guardar.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				new AlertDialog.Builder(prueba_de_ciclismo_activity.this)
						...
						.setPositiveButton("Sí", new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface dialog, int which) {
								...
								Toast.makeText(prueba_de_ciclismo_activity.this, "Datos guardados correctamente", Toast.LENGTH_SHORT).show();
							}
						})
	...
	public void generarPdf(){
		...
		double[][] datosEntrenamiento = {
			//[f][C]
			 {70.43,	76.58,	83.01,	89.16,	95.30,	100.61,	105.09},                   //% DE v4
			 {1, 1, 1, 1, 1, 1, 1},                                                       //% de reg
			 {1, 1, 1, 1, 1, 1, 1},                                                       //% de V4
			 {1, 1, 1, 1, 1, 1, 1},                                                       //vel por zonas
			 {1, 1, 1, 1, 1, 1, 1}                                                        //st
			 
	 	};

	 	//datosEntrenamiento[1][1] = datosEntrenamiento[0][1]/datosEntrenamiento[0][0];
		//datosEntrenamiento[1][2] = datosEntrenamiento[0][2]/datosEntrenamiento[0][0];	
	 	// Llenar la fila [1][] 
		...
		// LLenar la fila [3][]
		for (int i = 0; i < datosEntrenamiento[0].length; i++) {
    		datosEntrenamiento[3][i] = V4p*datosEntrenamiento[2][i]/100;
		}
		// LLenar la fila [4][]
		for (int i = 0; i < datosEntrenamiento[0].length; i++) {
    		datosEntrenamiento[4][i] = EtapasIniciales[0][5]/datosEntrenamiento[3][i];
		}

		String[][] primeraTabla = new String[3][7];
		//fila Min
		for (int i = 0; i < primeraTabla[0].length; i++) {
    		primeraTabla[0][i] = String.format("%d", (int)(datosEntrenamiento[4][i]/60));
		}
		//fila s
		int[] parchemin = new int[7];
		for (int i = 0; i < parchemin[0]; i++) {
    		parchemin[i] = (int) datosEntrenamiento[4][i]/60;
		}
		for (int i = 0; i < primeraTabla[0].length; i++) {
    		primeraTabla[1][i] = String.format("%d", (int)(datosEntrenamiento[4][i]-(parchemin[i]*60)));
		}
		//fila v4%
		for (int i = 0; i < primeraTabla[0].length; i++) {
    		primeraTabla[2][i] = String.format("%d", (int)(datosEntrenamiento[2][i]));
		}
